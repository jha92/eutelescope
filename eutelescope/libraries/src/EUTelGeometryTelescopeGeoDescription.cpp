...
void EUTelGeometryTelescopeGeoDescription::readSiPlanesLayout() {
  ...
  for (size_t iPlane_sz = 0; iPlane_sz < nPlanes; iPlane_sz++) {
    ...
    auto pmOneOrZero = [](double const & f) -> int {
      ...
      else throw std::runtime_error("EUTelGeometryTelescopeGeoDescription::readSiPlanesLayout "
				    "flip matrix exntry is not plus/minus one or zero");
    };
...
void EUTelGeometryTelescopeGeoDescription::readTrackerPlanesLayout() {
  ...
  for (size_t iLayer = 0; iLayer < nLayers; iLayer++) {
	...
	streamlog_out(ERROR5) << "Trying to add <active> with ID: " << activeID << " from layer: " << layerID 
			      << ", but same activeID/sensorID already existing on layer: "
...
EUTelGeometryTelescopeGeoDescription::EUTelGeometryTelescopeGeoDescription():
  _gearManager( marlin::Global::GEAR ), _siPlanesDefined(false), _telPlanesDefined(false), _siPlanesParameters(nullptr), 
  _siPlanesLayerLayout(nullptr), _trackerPlanesParameters(nullptr), _trackerPlanesLayerLayout(nullptr), _sensorIDVec(), 
  _isGeoInitialized(false), _isAnnulusGearInitialized(false), _geoManager(nullptr)
{
...
void EUTelGeometryTelescopeGeoDescription::readAnnulusGear() {
  ...
  if(_isAnnulusGearInitialized) {
	...
	for(int i=0;i<9;i++) {
	  ...
	  if(find_sensor) {
	    if(buffer.compare("pitchPhi") == 0) plane->setPitchPhi(value); 
	    if(buffer.compare("stereoAngle") == 0) plane->setStereoAngle(value); 
	    if(buffer.compare("rmin") == 0) plane->setRMin(value); 
	    if(buffer.compare("rmax") == 0) plane->setRMax(value); 
	    if(buffer.compare("order") == 0) plane->setOrder(value); 
	    if(buffer.compare("rCentre") == 0) plane->setRCentre(value); 
	    if(buffer.compare("Fx") == 0) plane->setFx(value); 
	    if(buffer.compare("Fy") == 0) plane->setFy(value); 
	  }
	}
	
	if(find_sensor) {
	  _annulusMap.insert(std::make_pair(sensorID, plane));     
	}
      }
...
void EUTelGeometryTelescopeGeoDescription::translateSiPlane2TGeo(TGeoVolume* pvolumeWorld, int SensorId ) {
  ...
  if(determinant == 1 or determinant == -1) { 
    streamlog_out(DEBUG5) << "SensorID: " << SensorId << ". Determinant =  " << determinant 
			  << "  This is the correct determinate for this transformation." << std::endl;   
  } else {
  ...
  if( name == "CAST" ) {
    ...
    //check annulus geometry planes
    std::map<int, EUTelAnnulus*>::iterator mapIt = _annulusMap.find(SensorId);
    if(mapIt != _annulusMap.end()) {
      auto annulusPlane  = mapIt->second;
      _pixGeoMgr->addCastedAnnulusPlane( SensorId, getPlaneNumberOfPixelsX(SensorId), getPlaneNumberOfPixelsY(SensorId),
					 getPlaneXSize(SensorId), getPlaneYSize(SensorId), getPlaneZSize(SensorId),
					 annulusPlane->getPitchPhi(), annulusPlane->getStereoAngle(), 
					 annulusPlane->getRMin(), annulusPlane->getRMax(), annulusPlane->getRCentre(), 
					 annulusPlane->getOrder(), getPlaneRadiationLength(SensorId), stVolName);
    } else {
      _pixGeoMgr->addCastedPlane( SensorId, getPlaneNumberOfPixelsX(SensorId), getPlaneNumberOfPixelsY(SensorId), 
				  getPlaneXSize(SensorId), getPlaneYSize(SensorId), getPlaneZSize(SensorId), 
				  getPlaneRadiationLength(SensorId), stVolName);
    }
  } else {
